...
COUNTRY_ALIASES = load_json_mapping("country_aliases.json")
COUNTRY_MAIN_AIRPORT = load_json_mapping("country_main_airport.json")
CITY_MAIN_AIRPORT = load_json_mapping("city_main_airport.json")
...
def country_name_to_code(text: str):
    ...
    # Handle common names that pycountry may not resolve the way users say them.
    ...
    # Detect a formal country name inside longer text, such as "trip to India".
...
def airport_country_matches(airport: dict, country_code: str) -> bool:
    ...
    # airportsdata may store either an ISO code or a country name depending on
    # the record, so compare against both forms.
...
def get_best_airport_for_country(country_code: str):
    # Prefer curated main airports first; they avoid odd matches in countries
    # with many regional airports.
    ...
    for iata, airport in AIRPORTS.items():
        ...
        if airport_country_matches(airport, country_code):
            ...
            # If no curated airport exists, choose the most likely major airport
            # using simple signals available in the airport metadata.
            score = 0
...
def resolve_location_to_iata(location: str):
    ...
    # City names can map to a preferred airport when the city has multiple
    # airports or nearby airport options.
    ...
    # Last resort: score airport database matches by city/name relevance.
    city_matches = []
...
def parse_route(query: str):
    ...
    # Direct IATA code route, for example: "DAC to NRT".
    ...
    # Fallback: infer route from mentioned countries/cities when explicit
    # "from/to" wording is missing.
...
def format_flight(flight: dict):
    """Convert one AviationStack flight record into readable text."""
...
def search_flights(query: str, limit: int = 10):
    ...
    # AviationStack uses optional filters; omitting both returns broad live
    # flight data, while including either side narrows the route.
    params = {
...
#     print(search_flights("5 day trip to Malaysia from India"))
#     print("\n" + "=" * 80 + "\n")
